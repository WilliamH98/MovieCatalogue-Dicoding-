...
public class TVShowsSearchViewModel extends ViewModel {
    private MutableLiveData<ArrayList<TVShows>> showsList = new MutableLiveData<>();

    public void setShows(String locale, final Context context, String querySearch) {
        TVShowsSearchViewModel.GetShowsServices showsServices = RetrofitClientInstance.getRetrofitClientInstanceSearch().create(TVShowsSearchViewModel.GetShowsServices.class);
        Call<APITVShowsModel> callList = showsServices.getShowsAPIData(String.format("tv?api_key=%s&language=%s&query=%s", BuildConfig.API_KEY, locale, querySearch));
